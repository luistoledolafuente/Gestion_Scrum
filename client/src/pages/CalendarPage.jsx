...
export function CalendarPage() {
  ...
  return <div className="page wide-page">
    <div className="page-heading calendar-heading"><div><span className="eyebrow">Agenda compartida</span><h1>Calendario</h1><p>Reuniones, demos y ceremonias del equipo.</p></div><div className="heading-actions"><SyncStatus {...polling} onRefresh={polling.refresh} compact /><button className="primary-button" onClick={openCreate}><Plus size={17} />Nueva reunión</button></div></div>
    ...
    {view === 'day' && <DayCalendar day={anchorDate} events={events} onEdit={openEdit} />}
    {view === 'week' && <WeekCalendar start={range.start} events={events} onEdit={openEdit} />}
    {view === 'month' && <MonthCalendar start={range.start} month={anchorDate.getMonth()} events={events} onEdit={openEdit} />}
