import { ArrowLeft, CalendarDays, Clock3, ExternalLink, ListChecks, Plus, Target } from 'lucide-react'
...
export function ProjectPage() {
  ...
  return <div className="page">
    ...
    <div className="project-heading"><div><div className="title-line"><h1>{project.name}</h1><StatusBadge status={project.status} /></div><p>{project.description}</p><div className="project-meta"><span><CalendarDays size={16} />{formatDate(project.startDate)} — {formatDate(project.endDate)}</span><span>Cliente: <b>{project.client.companyName || project.client.name}</b></span></div></div><div className="heading-actions"><SyncStatus {...polling} onRefresh={polling.refresh} compact /><button className="secondary-button" onClick={() => openModal('backlog')}><Plus size={16} />Backlog</button><button className="primary-button" onClick={() => openModal('sprint')}><Plus size={16} />Sprint</button><Link className="secondary-button" to={`/portal/${project.publicPortalToken}`} target="_blank">Ver portal <ExternalLink size={16} /></Link></div></div>
    ...
    <div className="two-column"><section className="panel"><div className="section-heading compact"><div><h2>Sprints</h2><p>Iteraciones del proyecto</p></div><button className="tiny-button" onClick={() => openModal('sprint')}><Plus size={14} />Añadir</button></div><SprintList sprints={project.sprints} /></section><section className="panel"><div className="section-heading compact"><div><h2>Backlog</h2><p>Items priorizados</p></div><button className="tiny-button" onClick={() => openModal('backlog')}><Plus size={14} />Añadir</button></div><div className="backlog-list">{project.backlogItems.map((item) => <div key={item.id}><span className={`type-dot type-${item.type}`} /><div><strong>{item.title}</strong><small>{item.type} · {item.storyPoints} pts</small></div><StatusBadge status={item.status} /></div>)}{!project.backlogItems.length && <div className="empty-inline">El backlog todavía está vacío.</div>}</div></section></div>
